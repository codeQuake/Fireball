		...
		<table class="table jsClipboardContainer" data-type="de.codequake.cms.stylesheet">
			<thead>
				<tr>
					...
					<th class="columnID columnStylesheetID{if $sortField == 'stylesheetID'} active {@$sortOrder}{/if}" colspan="2"><a href="{link controller='StylesheetList' application='cms'}pageNo={@$pageNo}&sortField=stylesheetID&sortOrder={if $sortField == 'stylesheetID' && $sortOrder == 'ASC'}DESC{else}ASC{/if}{/link}">{lang}wcf.global.objectID{/lang}</a></th>
			...
			<tbody>
				{foreach from=$objects item=sheet}
					<tr class="jsClipboardObject jsStylesheetRow">
						...
						<td class="columnTitle columnStylesheet"><a href="{link controller='StylesheetEdit' id=$sheet->stylesheetID application='cms'}{/link}">{$sheet->title}</a></td>
