<script data-relocate="true" src="{@$__wcf->getPath()}js/WCF.Poll{if !ENABLE_DEBUG_MODE}.min{/if}.js?v={@LAST_UPDATE_TIME}"></script>
...
<dl{if $errorField == 'pollOptions'} class="formError"{/if}>
	<dt><label for="pollQuestion">{lang}wcf.poll.question{/lang}</label></dt>
	<dd>
		<input type="text" name="pollQuestion" id="pollQuestion" value="{$pollQuestion}" class="long" maxlength="255" />
	</dd>

	<dt><label>{lang}wcf.poll.options{/lang}</label></dt>
...
<dl{if $errorField == 'pollEndTime'} class="formError"{/if}>
	<dt><label for="pollEndTime">{lang}wcf.poll.endTime{/lang}</label></dt>
	<dd>
...
<dl{if $errorField == 'pollMaxVotes'} class="formError"{/if}>
	<dt><label for="pollMaxVotes">{lang}wcf.poll.maxVotes{/lang}</label></dt>
	<dd>
